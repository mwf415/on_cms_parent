...
public class FrontUtils {
	...
	public static String getTplPath(HttpServletRequest request,
		...
		return solution + "/" + dir + "/"
				+ MessageResolver.getMessage(request, name) + Constants.TPL_SUFFIX;
	}
	...
	public static String getTplPath(MessageSource messageSource, String lang,
		...
		return solution + "/" + dir + "/"
				+ messageSource.getMessage(name, null, locale) + Constants.TPL_SUFFIX;
	}
	...
	public static String pageNotFound(HttpServletRequest request,
		...
		return getTplPath(request, site.getSolutionPath(),  Constants.TPLDIR_COMMON,
				PAGE_NOT_FOUND);
	}
	...
	public static String showMessage(HttpServletRequest request,
		...
		return getTplPath(request, site.getSolutionPath(),  Constants.TPLDIR_COMMON,MESSAGE_PAGE);
	}
	...
	public static void frontData(Map<String, Object> map, CmsSite site,
		...
		map.put(BASE, ctx);
		map.put(RES_SYS, ctx +  Constants.RES_PATH);
		String res = ctx +  Constants.RES_PATH + "/" + site.getPath() + "/"
				+ site.getTplSolution();
		String mobileRes = ctx +  Constants.RES_PATH + "/" + site.getPath() + "/"
	...
	public static void includePagination(CmsSite site,
		...
		if (!StringUtils.isBlank(sysPage)) {
			...
			String tpl = getTplPath(site.getSolutionPath(),  Constants.TPLDIR_STYLE_LIST,
					userPage);
			env.include(tpl, UTF8, true);
		} else {
			// 没有包含分页
	...
	public static void includeTpl(String tplName, CmsSite site,
		...
		if (StringUtils.isBlank(subTpl)) {
			tpl = getTplPath(site.getSolutionPath(),  Constants.TPLDIR_TAG, tplName);
		} else {
			tpl = getTplPath(site.getSolutionPath(),  Constants.TPLDIR_TAG, tplName + "_"
					+ subTpl);
		}
		env.include(tpl, UTF8, true);
	}
