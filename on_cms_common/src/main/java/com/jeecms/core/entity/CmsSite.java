...
public class CmsSite extends BaseCmsSite {
	...
	public String getUrl() {
		...
		return url.toString();
		/*
		 * if (getStaticIndex()) { return getUrlStatic(); } else { return
		 * getUrlDynamic(); }
		 */
	}
	...
	public StringBuilder getMobileUrlBuffer(boolean dynamic, Boolean whole, boolean forIndex) {
		boolean relative = whole != null ? !whole : getRelativePath();
		String ctx = getContextPath();
		StringBuilder url = new StringBuilder();
		if (!relative) {
	 ...
	 * @return
	 */
	public JSONObject getAttrForStat(Integer siteId) {
		if ((siteId != null && siteId == getId()) || siteId == null) {
			JSONObject json = new JSONObject();
			Map<String, String> map = getAttr();
			if (map.get("siteName") != null && map.get("userName") != null) {
				json.put("id", getId());
				json.put("siteName", map.get("siteName") != null ? map.get("siteName") : "");
				json.put("userName", map.get("userName") != null ? map.get("userName") : "");
				json.put("password", map.get("password") != null ? map.get("password") : "");
				json.put("tjToken", map.get("tjToken") != null ? map.get("tjToken") : "");
				json.put("tjSiteId", map.get("tjSiteId") != null ? map.get("tjSiteId") : "");
				return json;
			} else {
				return null;
			}
	 ...
	 */
	public CmsSite(Integer id, com.jeecms.core.entity.CmsConfig config, String domain,
                   ...
                   Byte finalStep, Byte afterCheck, Boolean relativePath,
                   Boolean resycleOn) {

		super(id, config, domain, path, name, protocol, dynamicSuffix, staticSuffix, indexToRoot, staticIndex,
				localeAdmin, localeFront, tplSolution, finalStep, afterCheck, relativePath, resycleOn);
	}
